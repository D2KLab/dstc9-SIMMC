...
class BertCollate():
    ...
    def collate_fn(self, batch):
        ...
        for resp in wizard_resp:
            assert len(resp) == 0, 'Ground truth response for the k-th turn should be hidden!'
...
def preprocess(test_dataset, args):

    save_path = '{}/{}'
    collate = BertCollate('bert-base-uncased')
    metadata_ids = collate.metadata2ids(test_dataset.processed_metadata)
    ...
    params = {'batch_size': len(test_dataset),
            'shuffle': False,
            'num_workers': 0}
    assert params['batch_size'] == len(test_dataset) and not params['shuffle'], 'Keep batch size to max and shuffle to False to avoid problems during training'
    testloader = DataLoader(test_dataset, **params, collate_fn=collate.collate_fn)

    start_t = time.time()

    save_data_on_file(loader=testloader, save_path=save_path.format(args.save_path, 'teststd_response_retrieval_data.dat'))
...
if __name__ == '__main__':
    ...
    dataset_path = '{}/fashion_{}_dials_public.json'
    actions_path = '{}/fashion_{}_dials_api_calls.json'
    candidates_path = '{}/fashion_{}_dials_retrieval_candidates_public.json'

    test_dataset = SIMMCDatasetForResponseGeneration(data_path=dataset_path.format(args.simmc_folder, 'teststd'), 
                                                    metadata_path=args.metadata, 
                                                    actions_path=actions_path.format(args.actions_folder, 'teststd'), 
                                                    candidates_path=candidates_path.format(args.simmc_folder, 'teststd'),
                                                    return_only_last_turn=True)

    preprocess(test_dataset, args)
